...
class SpiltBillScreen extends React.Component {
    ...
    deleteItem = async (item) => {
        ...
        this.setState({enableDeleteItem: true})
        ...
        await dispatchFetchRequestWithOption(url, {
            ...
            setTimeout(() => {
                this.setState({enableDeleteItem: false})
            }, 500)
        })
    }
                                                        ...
                                                        this.state.splitOrderData?.lineItems?.map((item, index) => {
                                                            return (
                                                                ...
                                                                <TouchableOpacity key={index} style={[{backgroundColor: '#d6d6d6', color: '#000'}, {marginBottom: 16, borderRadius: 10}, (!!this.state?.choosenItem?.[item.lineItemId] && {backgroundColor: customMainThemeColor})]}
                                                                    activeOpacity={0.8}
                                                                    disabled={this.state.enableDeleteItem}
                                ...
                                <View style={{flex: 8}}>
                                    <ScrollView style={{flex: 1}}>
                                        {order?.lineItems?.length > 0 ?
                                            order?.lineItems?.map((item, index) => {
                                                return (
                                                    <TouchableOpacity key={index} style={[styles.rowFront, themeStyle, {backgroundColor: customBackgroundColor}]}
                                                        disabled={this.state.enableAddItem}
                                                        onPress={() => {this.addItem(item)}}>
